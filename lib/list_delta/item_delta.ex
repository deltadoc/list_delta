defprotocol ListDelta.ItemDelta do
  @moduledoc """
  Item delta represents a delta of list item value.
  ...
  for any new value type you want lists to support.
  """

  @fallback_to_any true

  alias ListDelta.{Transformation, Application}

  @typedoc """
  Result of `&ListDelta.appy/2` operation.
  ...
  list state, we can transform the operations of delta A such that the state of
  the list after applying delta A and then delta B is the same as after applying
  delta B and then the transformation of delta A against delta B:

    S ○ Oa ○ transform(Ob, Oa) = S ○ Ob ○ transform(Oa, Ob)

  The function also takes a third `t:ListDelta.Transformation.priority/0`
  ...
  with `t:ListDelta.Application.item_path/0` and
  `t:ListDelta.Application.error_reason/0`.
  """
  @spec apply(any, any) :: application_result
  def apply(state, delta)
end

defimpl ListDelta.ItemDelta, for: Any do
  @moduledoc """
